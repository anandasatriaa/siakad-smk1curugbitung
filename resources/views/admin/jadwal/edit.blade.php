...
@section('content')
    <div class="container-xxl flex-grow-1 container-p-y">
        <h4 class="fw-bold py-3 mb-4">
            <span class="text-muted fw-light"><a href="{{ route('admin.jadwal.index') }}">Data Jadwal</a> /</span> Edit Data
        </h4>
            ...
            <div class="col-xl-12">
                <div class="card mb-4">
                    <div class="card-header d-flex justify-content-between align-items-center">
                        <h5 class="mb-0">Form Edit Jadwal Pelajaran ({{ $jadwal->kelas->nama_kelas }} -
                            {{ $jadwal->hari }})</h5>
                    </div>
                    <div class="card-body">
                            ...
                            <div id="mapel-container">
                                @foreach ($jadwals as $index => $item)
                                    <div class="row mapel-row align-items-end mb-3">
                                            ...
                                            <select class="form-select" name="mapel_id[]" required>
                                                ...
                                                @foreach ($mapels as $mapel)
                                                    <option value="{{ $mapel->id }}"
                                                        ...
                                                        {{ $mapel->nama_mapel }} ({{ $mapel->kode_mapel }})</option>
                                                @endforeach
                                            </select>
                                        </div>
                                        <div class="col-md-3">
                                        ...
                                        <div class="col-md-2">
                                            <button type="button"
                                                class="btn btn-danger btn-remove-mapel {{ $index === 0 ? 'd-none' : '' }}"><i
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            btnAdd.addEventListener('click', function() {
                ...
                row.querySelectorAll('select, input').forEach(input => {
                    input.value = '';
                    // Remove selected attribute from options if any
                    if (input.tagName === 'SELECT') {
                        input.querySelectorAll('option').forEach(opt => opt.removeAttribute(
                            'selected'));
                    }
                });
                // Show remove button
